...
function exportYieldSeries(chainHistory: ChainHistory, contract: string, prefix: string) {
    const keys = Object.keys(chainHistory[contract]).sort((a, b) => Number(a) - Number(b));
    let previousLevel = 0;
    const rawRevenue = {};
    for (const level of keys) {
    ...
    fs.writeFileSync(`./${prefix}_dollarizedRevenue.json`, JSON.stringify(dollarizedRevenue, undefined, 4));
    ...
    fs.writeFileSync(`./${prefix}_dollarizedRevenue.csv`, csvDollarizedRevenue);
}

async function run() {
    const historyFiles: string[] = glob.sync('./data/*_history.json');
    const prefixes = historyFiles.map(n => n.replace('data/', '').replace('_history.json', ''));

    for (const prefix of prefixes) {
        const historyFile = `./data/${prefix}_history.json`;
        const chainHistory: ChainHistory = JSON.parse(fs.readFileSync(historyFile).toString());

        for(const contract of Object.keys(chainHistory) ) {
            exportYieldSeries(chainHistory, contract, prefix + '_' + contract);
            exportSupplySeries(chainHistory, contract);
        }
    }
}
