    ...
    for (const row of data) {
        ...
        if (row['parameter']['entrypoint'] === 'addLiquidity') {
            ...
            operationHistory[row['level']].push({
                ...
                operator: row['sender']['address'],
                input: 'token',
                amount: JSONPath({ path: '$.value.tokensSold', json: row['parameter'] })[0]
            });
        } else { // xtzToToken
            operationHistory[row['level']].push({
                hash: row['hash'],
...
async function getContractInternalBalanceHistory(contract: string): Promise<BalanceMap> {
    ...
    while (lastRecodedBlock > limit) { // data comes in descending
        ...
        const result = await fetch(`${url}?${params}`, { method: 'GET', cache: 'no-cache' });
        const data = await result.json();
        // console.log('getContractInternalBalanceHistory', `${url}?${params}`, JSON.stringify(data));
        ...
        for (const row of data) {
            ...
            balanceHistory[row['level']] = JSONPath({ path: '$.xtzPool', json: row['value'] })[0];
        }
        if (lowestBlock == lastRecodedBlock) { break; }
        lowestBlock = lastRecodedBlock;
    }
...
async function importInternalCoinBalanceHistory(chainHistory: ChainHistory, contract: string, limit: string | number): Promise<ChainHistory> {
    ...
    let previousLastRecodedBlock = 0;
    ...
    while (lastRecodedBlock + 1000 < limit) { // TODO: 1000 is needs to be better
        ...
        if (previousLastRecodedBlock == lastRecodedBlock) {
            break;
        }
        previousLastRecodedBlock = lastRecodedBlock;
